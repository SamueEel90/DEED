...
export function Shell({ title, step, total, onBack, children, footer }) {
  return (
...
export function Patka({ onBack, onNext, next = "Pokračovať", canNext = true, loading }) {
  return (
...
// ---- popisok poľa ----
export function Pole({ label, hint, children }) {
  return (
...
export function TextPole({ label, hint, value, onChange, placeholder, type = "text", inputMode, maxLength }) {
  return (
...
// ---- prepínač (toggle) ----
export function Prepinac({ on, onToggle, title, desc }) {
  return (
    <div
      onClick={onToggle}
...
function CheckRiadok({ label, on, onClick, akcent }) {
  return (
...
export function Accordion({ skupiny, jeVybrane, onToggle, onVlastny, akcent = C.blueL }) {
  const [otvorene, setOtvorene] = useState(null);
  return (
    <>
      {skupiny.map((s) => {
...
function VlastnaPolozka({ onAdd, akcent }) {
...
// ============================================================
export function KrokTelefonSms({ step, total, onBack, typ = "aktivny", vyzadujEmail = false, onHotovo, toast }) {
  const [faza, setFaza] = useState("zadanie"); // zadanie | overenie
  const [tel, setTel] = useState("+421 ");
  const [email, setEmail] = useState("");
  const [kod, setKod] = useState("");
  const [demoKod, setDemoKod] = useState(null);
  ...
  const over = async () => {
    ...
    setLoading(true);
    try {
      const ucet = await vytvorUcet({ typ, telefon: tel.trim(), email: vyzadujEmail ? email.trim() : null });
...
// ============================================================
export function KrokZabezpecenie({ step, total, onBack, ucetId, onHotovo, toast }) {
