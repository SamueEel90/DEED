...
export function Registracia({ onHotovo }) {
  const [typ, setTyp] = useState(null); // null | "osoba" | "charita"
  const [toastText, setToastText] = useState("");

  const zhasni = useRef(null);
  const toast = (text) => {
    setToastText(text);
    clearTimeout(zhasni.current);
...
function VidlickaTyp({ onPick, onPreskoc, toast }) {
  return (
