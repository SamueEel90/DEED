...
// rozsahy rebríčka
const ROZSAHY = ["Štvrť", "Mesto", "Celá SR"];
...
export default function ModulTop({ wide }) {
  const [screen, setScreen] = useState("top"); // top | fun | profil
  const [subjekt, setSubjekt] = useState(null);
  const [hlaska, setHlaska] = useState(null);
  const [rozsah, setRozsah] = useState("Štvrť");
  ...
  const otvorProfil = (s) => { setSubjekt(s); setScreen("profil"); scrollHore(); };

  if (screen === "fun") return <div style={{ minHeight: "100%" }}>{obal(<FunZona onBack={() => setScreen("top")} toast={toast} />)}{hlaska && <Toast text={hlaska} />}</div>;
  if (screen === "profil" && subjekt) return <div style={{ minHeight: "100%" }}>{obal(<CudziProfil subjekt={subjekt} toast={toast} onBack={() => setScreen("top")} />)}{hlaska && <Toast text={hlaska} />}</div>;
