...
function SubScreen({ nazov, toast, onBack }) {
  const [retaz, setRetaz] = useState(null); // ručná Reťaz dobra pri menšom skutku {odmena}
  let obsah;
...
function PriateliaScreen({ toast, onBack }) {
  const [qr, setQr] = useState(null);     // "pozvanka" | "osobny"
  const [ziadosti, setZiadosti] = useState([{ id: "p1", meno: "Peter K.", ini: "P", info: "3 spoloční priatelia" }]);
  const vybav = (id, ok) => { setZiadosti((z) => z.filter((x) => x.id !== id)); toast(ok ? "Priateľstvo prijaté — vzájomný súhlas" : "Žiadosť odmietnutá"); };
...
const TEMY = ["Šport", "Eko", "Zdravie", "Art", "Učenie", "Komunita", "Zvieratá", "Senior"];

function NastaveniaScreen({ toast, onBack, onNotif }) {
  const { svetly, prepni } = useMotiv();
  const [jazyk, setJazyk] = useState("SK");
  const [rezim, setRezim] = useState("verejny");          // verejný / anonym (§13.1 ochrana)
  ...
  const [temy, setTemy] = useState(["Šport", "Eko", "Zdravie", "Art"]);
  const toggleTema = (t) => setTemy((s) => s.includes(t) ? s.filter((x) => x !== t) : [...s, t]);

  const Sekcia = ({ children }) => <div style={{ fontSize: 10.5, letterSpacing: ".5px", color: C.textTer, fontWeight: 700, margin: "18px 0 8px" }}>{children}</div>;
  const Riadok = ({ children, onClick }) => <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 12, background: C.surface, border: `1px solid ${C.line}`, borderRadius: 12, padding: "13px 14px", marginBottom: 8, fontSize: 14.5, cursor: onClick ? "pointer" : "default" }}>{children}</div>;
